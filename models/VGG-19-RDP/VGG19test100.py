...
net = VGGNet19(num_classes=100).to(device)
criterion = nn.CrossEntropyLoss()
optimizer = optim.SGD(net.parameters(), lr=lr, momentum=0.9, weight_decay=1e-4)
best_acc = 65
print ("Start Training !")
start = time.perf_counter()
with open ("acc_vgg19net.txt","w") as file1 :
    with open ("log_vgg19net.txt","w") as file2 :
        for epoch in range (pre_epoch, epochs) :
            ...
            with torch.no_grad() :
                ...
                if acc > best_acc :
                    if acc > 70 :
                        print ("Saving Model--->")
                        torch.save(net.state_dict(), 'vggnet19model'+str(epoch+1)+'.pth')
                    file3 = open("best_acc_vgg19net.txt","a")
...
time_out = end - start
with open ("time_costvgg19.txt","w") as filet :
