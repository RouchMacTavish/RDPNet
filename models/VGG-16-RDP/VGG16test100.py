...
trainset = torchvision.datasets.CIFAR100(root='./data2', train=True, download=True, transform=transform_train )
trainloader = torch.utils.data.DataLoader(trainset, batch_size = batch_size, shuffle=True, num_workers=2)

testset = torchvision.datasets.CIFAR100(root='./data2', train=False, download=True, transform=transform_test)
testloader = torch.utils.data.DataLoader(testset, batch_size=100, shuffle=False, num_workers=2)

classes = ('plane', 'car', 'bird', 'cat', 'deer', 'dog', 'frog', 'horse', 'ship', 'truck')


net = VGGNet16(num_classes=100).to(device)
...
with open ("acc_vgg16net.txt","w") as file1 :
    with open ("log_vgg16net.txt","w") as file2 :
        for epoch in range (pre_epoch, epochs) :
            ...
            with torch.no_grad() :
                ...
                if acc > best_acc :
                    if acc > 70 :
                        print ("Saving Model--->")
                        torch.save(net.state_dict(), 'vggnet16model'+str(epoch+1)+'.pth')
                    file3 = open("best_acc_vgg16net.txt","a")
...
time_out = end - start
with open ("time_costvgg16.txt","w") as filet :
